...
async function applyFixedAnalyticsSchema() {
  console.log('🗄️  Applying Fixed Phase 4C Analytics Schema...\n');

  try {
    // Read the fixed analytics schema file
    const schemaPath = path.join(process.cwd(), 'database', 'phase4c-analytics-schema-fixed.sql');
    
    if (!fs.existsSync(schemaPath)) {
      throw new Error('Fixed analytics schema file not found. Please ensure database/phase4c-analytics-schema-fixed.sql exists.');
    }
    
    const schemaSQL = fs.readFileSync(schemaPath, 'utf8');
    console.log('✅ Fixed schema file read successfully');
    ...
    if (errorCount === 0) {
      console.log('\n🎉 Fixed analytics schema applied successfully!');
    } else {
    ...
    console.error('❌ Failed to apply fixed analytics schema:', error);
    throw error;
  } finally {
...
// Run the fixed schema application
applyFixedAnalyticsSchema().catch(console.error);
