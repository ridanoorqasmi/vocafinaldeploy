...
const { runRule } = require('../lib/runRule');
...
async function testRuleEngine() {
  ...
  try {
    ...
    try {
      ...
      console.log(`⚠️  Dry run failed (expected for test data): ${error.message || 'Unknown error'}`);
    }
    ...
    try {
      ...
      console.log(`⚠️  Rule execution failed (expected for test data): ${error.message || 'Unknown error'}`);
    }
